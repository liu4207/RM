   ...
   void Chassis_task(void const *pvParameters)
{
    ...
    for(;;)				//底盘运动任务
    {     
 			Vx=rc_ctrl.rc.ch[2];// left right
			Vy=rc_ctrl.rc.ch[3];//forward
			Wz=rc_ctrl.rc.ch[4];
			chassis_motol_speed_calculate();
			Motor_Speed_limiting(motor_speed_target,1000);
...
void chassis_motol_speed_calculate()
{
	  ...
	  motor_speed_target[0] =  Vy+Vx-Wz;//CHAS_LF 2
    motor_speed_target[1] =  Vy-Vx+Wz;//CHAS_RF 3
    motor_speed_target[2] = Vy+Vx+Wz;// CHAS_RB 0 
    motor_speed_target[3] =  Vy-Vx-Wz;//CHAS_LB 1
}
//运动解算
//速度限制函数
