...
{
    rc_ctrl.rc.ch[0] = (rxBuf[0] | (rxBuf[1] << 8)) & 0x07ff;        //!< Channel 0  中值为1024，最大值1684，最小值364，波动范围：660   右侧左右
	rc_ctrl.rc.ch[1] = (((rxBuf[1] >> 3)&0xff) | (rxBuf[2] << 5)) & 0x07ff; //!< Channel 1			右侧上下  
    rc_ctrl.rc.ch[2] = (((rxBuf[2] >> 6)&0xff) | (rxBuf[3] << 2) |          //!< Channel 2 			左侧左右
                         (rxBuf[4] << 10)) &0x07ff;
	rc_ctrl.rc.ch[3] = (((rxBuf[4] >> 1)&0xff) | (rxBuf[5] << 7)) & 0x07ff; //!< Channel 3 				左侧上下
