...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan) // 接受中断回调函数
{
  ...
  if (hcan->Instance == CAN1)
  {
    ...
    if ((rx_header.StdId >= 0x201)                                 // 201-205
        && (rx_header.StdId < 0x206))                              // 判断标识符，标识符为0x200+ID
    {
      uint8_t index = rx_header.StdId - 0x201; // get motor index by can_id  0.1.2.3 4拨弹 8yaw
...
//      motor_info_chassis[index].real_angle = motor_info_chassis[index].rotor_angle * 360.0 / 8192;
    ...
    if ((rx_header.StdId >= FEEDBACK_ID_BASE_6020) &&              // 205-211,注意把ID调成大于3,不然就会和读取3508的函数产生冲突
        (rx_header.StdId < FEEDBACK_ID_BASE_6020 + MOTOR_MAX_NUM)) // 判断标识符，标识符为0x204+ID   0x204+5=ox209
    {
      uint8_t index = rx_header.StdId - FEEDBACK_ID_BASE_6020+4; // get motor index by can_id  index=4+4=8
      motor_info_chassis[index].rotor_angle = ((rx_data[0] << 8) | rx_data[1]);
      motor_info_chassis[index].rotor_speed = ((rx_data[2] << 8) | rx_data[3]);
      motor_info_chassis[index].torque_current = ((rx_data[4] << 8) | rx_data[5]);
      motor_info_chassis[index].temp = rx_data[6];
//      motor_info_chassis[index].real_angle = motor_info_chassis[index].rotor_angle * 360.0 / 8192;
  ...
  if (hcan->Instance == CAN2)
  {
    ...
    if ((rx_header.StdId >= 0x205)                                 // 5 6 7
        && (rx_header.StdId <=0x207))                              // 判断标识符，标识符为0x200+ID
    {
      uint8_t index = rx_header.StdId - 0x200; //200是此处的特例，要不数组重复
      motor_info_chassis[index].rotor_angle = ((rx_data[0] << 8) | rx_data[1]);// 5 6 shoot 7 pitch
      motor_info_chassis[index].rotor_speed = ((rx_data[2] << 8) | rx_data[3]);
      motor_info_chassis[index].torque_current = ((rx_data[4] << 8) | rx_data[5]);
      motor_info_chassis[index].temp = rx_data[6];
//      motor_info_chassis[index].real_angle = motor_info_chassis[index].rotor_angle * 360.0 / 8192;
...
//    {
//      uint8_t index = rx_header.StdId - FEEDBACK_ID_BASE_6020-1; // get motor index by can_id 11-5-1=5
//      motor_info_chassis[index].rotor_angle = ((rx_data[0] << 8) | rx_data[1]);
//      motor_info_chassis[index].rotor_speed = ((rx_data[2] << 8) | rx_data[3]);
//      motor_info_chassis[index].torque_current = ((rx_data[4] << 8) | rx_data[5]);
//      motor_info_chassis[index].temp = rx_data[6];
...
void set_pitch_current(uint8_t id_range, int16_t v1, int16_t v2, int16_t v3, int16_t v4)
{
  CAN_TxHeaderTypeDef tx_header;
  uint8_t tx_data[8];
